...
class Model(nn.Module):
    def __init__(self, n_rel, n_ent, n_token, n_link_label, dim_embs, node_dropout, n_layers, score_dropout):
        ...
        # for entity tuples.
        self.ent_encoder = nn.Embedding(n_ent, dim_embs["ent"])
        nn.init.normal_(self.ent_encoder.weight, std=1/dim_embs["ent"]**0.5)
...
def eval_MAP(model, items, indmap, arities, args):
    ...
    for i_batch in range(n_batch):
        ...
        emb_ent_tups = emb_ent_tups.view(len(ent_tups[start:end]), emb_ent_tups.size(1) * emb_ent_tups.size(2))
